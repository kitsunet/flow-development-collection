...
class Utility {
	 ...
	 */
	static public function getComposerManifest($manifestPath, $configurationPath = NULL) {
	...
	public static function isFlowPackageType($packageType) {
		return (strpos($packageType, 'typo3-flow-') === 0);
	}
	...
	public static function writeComposerManifest($manifestPath, $packageKey, array $composerManifestData = []) {
		...
		if (!isset($manifest['autoload']) || empty($manifest['autoload'])) {
